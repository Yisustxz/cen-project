...
from space_shooter.core.constants import (
    GAME_WIDTH, GAME_HEIGHT, SCREEN_SIZE, RED, WHITE, 
...
class SpaceShooterGame(GameEngine):
    ...
    def __init__(self):
        ...
        super().__init__(GAME_WIDTH, GAME_HEIGHT, GAME_TITLE, FPS)
    ...
    def on_render_background(self):
        ...
        for bg_x in range(0, GAME_WIDTH, bg.get_width()):
            for bg_y in range(0, GAME_HEIGHT, bg.get_height()):
                self.game_window.blit(bg, (bg_x, bg_y))

    def on_render_foreground(self):
        """Renderiza elementos en primer plano y UI."""
        # Obtener el jugador
        player = self.objects_manager.get_objects_by_type("player")[0]
        
        # Dibujar daño si el jugador ha sido golpeado
        player.draw_damage(self.game_window)
            
        # Renderizar HUD
        if self.gameover:
            # Mostrar pantalla de game over con el HUD
            self.hud.render_game_over(self.game_window)
        else:
            # Mostrar HUD normal durante el juego
            self.hud.render(self.game_window, player, self.debug_mode)
